...
2. **Adapter serving** (implemented, tested against the fake/mocked backend):
   set `adapter_path` on a model in `configs/models.yaml` and the llama_cpp
   backend passes it to `llama-cpp-python` as `lora_path` at load time. A
   configured-but-missing adapter file fails the load with an actionable error
...
3. **Place the adapter locally** (Git-ignored), e.g.
   `models/adapters/april-brain-lora.gguf`, and set `adapter_path` for the
   model in `configs/models.yaml`.

4. **Verify like any other real-model change** — adapters are part of the real
   model surface, so the normal ladder applies:
...
- **Adapter quality gating is manual.** The `model_adapters` DB table exists
  for bookkeeping, but APRIL does not auto-evaluate or auto-activate adapters.
  Nothing claims a trained adapter is "better"; you decide after reviewing
  eval output.
...
Until you complete steps 1–4 on this Mac, LoRA serving is *wired but
unverified* — treat `adapter_path` the same way as any other real-model
